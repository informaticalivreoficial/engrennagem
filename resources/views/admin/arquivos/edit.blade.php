...
<div class="row mb-2">
    <div class="col-sm-6">
        <h1><i class="fas fa-search mr-2"></i>Editar Arquivo</h1>
    </div>
    <div class="col-sm-6">
...
<form action="{{ route('arquivos.update',['id' => $arquivo->id]) }}" method="post" enctype="multipart/form-data" autocomplete="off">
@csrf
@method('PUT')       
<div class="row">            
<div class="col-12">
    ...
    <div class="tab-pane fade show active" id="custom-tabs-four-home" role="tabpanel" aria-labelledby="custom-tabs-four-home-tab">
            ...
            <div class="col-12 col-md-6 col-lg-4"> 
                <div class="form-group">
                    <div class="thumb_user_admin">
                        <img id="preview" src="{{$arquivo->cover()}}" alt="{{ old('titulo') ?? $arquivo->titulo }}" title="{{ old('titulo') ?? $arquivo->titulo }}"/>
                ...
                <div class="row mb-2">
                    <div class="col-12 col-md-8 col-lg-8"> 
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="titulo" value="{{ old('titulo') ?? $arquivo->titulo }}">
                        </div>
                    </div>
                    <div class="col-12 col-md-4 col-lg-4"> 
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control">
                                <option value="1" {{ (old('status') == '1' ? 'selected' : ($arquivo->status == '1' ? 'selected' : '')) }}>Ativo</option>
                                <option value="0" {{ (old('status') == '0' ? 'selected' : ($arquivo->status == '0' ? 'selected' : '')) }}>Inativo</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-12 col-md-6 col-lg-6"> 
                        <div class="form-group">
                            ...
                            <select name="tipo" class="form-control">
                                <option value="ai" {{ (old('tipo') == 'ai' ? 'selected' : ($arquivo->tipo == 'ai' ? 'selected' : '')) }}>Adobe Illustrator (.ai)</option>
                                <option value="png" {{ (old('tipo') == 'png' ? 'selected' : ($arquivo->tipo == 'png' ? 'selected' : '')) }}>Adobe Fireworks (.png)</option>
                                <option value="cdr" {{ (old('tipo') == 'cdr' ? 'selected' : ($arquivo->tipo == 'cdr' ? 'selected' : '')) }}>Corel Draw (.cdr)</option>
                                <option value="xls" {{ (old('tipo') == 'xls' ? 'selected' : ($arquivo->tipo == 'xls' ? 'selected' : '')) }}>Microsoft Excel (.xls)</option>
                                <option value="doc" {{ (old('tipo') == 'doc' ? 'selected' : ($arquivo->tipo == 'doc' ? 'selected' : '')) }}>Microsoft Word (.doc)</option>
                                <option value="docx" {{ (old('tipo') == 'docx' ? 'selected' : ($arquivo->tipo == 'docx' ? 'selected' : '')) }}>Microsoft Word (.docx)</option>
                                <option value="dotx" {{ (old('tipo') == 'dotx' ? 'selected' : ($arquivo->tipo == 'dotx' ? 'selected' : '')) }}>Modelo de documento Word (.dotx)</option>
                                <option value="pdf" {{ (old('tipo') == 'pdf' ? 'selected' : ($arquivo->tipo == 'pdf' ? 'selected' : '')) }}>Arquivo PDF (.pdf)</option>
                                <option value="zip" {{ (old('tipo') == 'zip' ? 'selected' : ($arquivo->tipo == 'zip' ? 'selected' : '')) }}>Arquivo Compactado (.zip)</option>
                            </select>
                        </div>
                    </div>           
                    <div class="col-12 col-md-6 col-lg-6"> 
        ...
        <div class="row mb-2">
            <div class="col-12">   
                ...
                <textarea class="form-control" rows="5" name="content">{{ old('content') ?? $arquivo->content }}</textarea>                                                                                     
            </div>            
        </div>
