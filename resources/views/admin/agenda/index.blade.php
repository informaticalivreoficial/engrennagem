...
@section('title', "Gerenciar Agenda")

@section('content_header')
<div class="row mb-2">
    <div class="col-sm-6">
        <h1><i class="fas fa-search mr-2"></i> Gerenciar Agenda</h1>
    </div>
    <div class="col-sm-6">
            ...
            @if(!empty($eventos) && $eventos->count() > 0)
                <table id="example1" class="table table-bordered table-striped projects">
                    ...
                    <tbody> 
                        ...
                        <tr style="{{ ($evento->status == '1' ? '' : 'background: #fffed8 !important;')  }}">                            
                            <td class="text-center">
                                <a href="{{ $evento->cover() }}" data-title="{{$evento->titulo}}" data-toggle="lightbox">
                                    <img width="60" alt="{{$evento->titulo}}" src="{{$evento->cover()}}">
                                </a>
                            </td>
                            <td>{{$evento->titulo}}</td>
                            ...
                            <td>
                                ...
                                <a href="{{ route('agenda.edit', [ 'id' => $evento->id ]) }}" class="btn btn-xs btn-default"><i class="fas fa-pen"></i></a>
                                @if ($evento->link)
                                    <a target="_blank" href="{{$evento->link}}" class="btn btn-xs btn-info text-white"><i class="fas fa-search"></i></a>
                                @endif                                
                                <button type="button" class="btn btn-xs btn-danger text-white j_modal_btn" data-id="{{$evento->id}}" data-toggle="modal" data-target="#modal-default" {{($evento->id == 555 ? 'disabled' : '')}}><i class="fas fa-trash"></i></button>
                            </td>
                        </tr>                            
                        @endforeach
                    </tbody>                
...
<div class="modal fade" id="modal-default">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            @csrf
            @method('DELETE')
            <input id="id_evento" name="evento_id" type="hidden" value=""/>
           ...
           $.ajaxSetup({
                ...
                $.ajax({
                    ...
                    success:function(data) {
                        if(data.error){
                            ...
                            $('#frm').prop('action','{{ route('agenda.deleteon') }}');
                        }else{
                            $('#frm').prop('action','{{ route('agenda.deleteon') }}');
                        }
                    }
            ...
            $('.toggle-class').on('change', function() {
                ...
                $.ajax({
                    type: 'GET',
                    dataType: 'JSON',
                    url: '{{ route('agenda.agendaSetStatus') }}',
                    data: {
                        'status': status,
                        'id': evento_id
                    },
